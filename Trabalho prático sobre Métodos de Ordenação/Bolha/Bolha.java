...
public class Bolha {
	...
	public static int qtdLinhas(ArquivoTextoLeitura leitura) {
		int qtd = 0;
		String dadosLidos = new String();
		leitura.abrirArquivo("/tmp/jogadores.txt");

		leitura.ler();
		dadosLidos = leitura.ler();
		while (dadosLidos != null) {
			qtd++;
			dadosLidos = leitura.ler();
		}
	...
	private Jogador[] method(Jogador[] array, int n) {
		...
		for (int i = (n - 1); i > 0; i--) {
			for (int j = 0; j < i; j++) {
				...
				teste1 = array[j].getCidadeNascimento().trim().length() == 0;
				teste2 = array[j + 1].getCidadeNascimento().trim().length() == 0;
				menor1 = array[j].getCidadeNascimento().compareTo(array[j + 1].getCidadeNascimento()) > 0;
				iguais = array[j].getCidadeNascimento().compareTo(array[j + 1].getCidadeNascimento()) == 0;
				menorNome = array[j].getNome().compareTo(array[j + 1].getNome()) > 0;

				if ((teste1 && !teste2) || (!teste2 && menor1) || (iguais && menorNome)) {
					trocas++;
